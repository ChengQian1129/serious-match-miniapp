...
说明：本轮 RC hardening 工作树已修复边界状态和生成链；尚未推送本轮改动、重新上传体验版或重新部署云函数。
...
- GitHub Actions `release checks`：`NOT VERIFIED`；基线失败根因是 `check-public-language.js` 默认 locale 排序，已改为确定性 comparator，待推送后验证。
...
已有版本 `3.0.0` 体验版上传记录，但本轮最新布局、恢复、反馈、编辑边界和云函数生成物尚未重新上传/部署。剩余是推送后 GitHub Actions、微信工具/真机与后台人工验收：真实设备回归、弱网和双设备、云开发权限核对、隐私资料核对，以及提交体验版/审核版。
