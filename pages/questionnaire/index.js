...
Page({
  ...
  onLoad(query) {
    ...
    this.setData(Object.assign({ chapterNumber: Math.max(1, ['C1', 'C2', 'C3', 'C4', 'C5', 'C6'].indexOf(this.chapterId) + 1), moduleTitle: this.chapter.title, moduleInstruction: this.chapter.instruction, chapterIntro: getChapterCopy(this.chapterId), questionCount: 8, currentQuestion: index, showChapterIntro: index === 0, motionClass: getMotionClass(query.direction) }, this.getQuestionState(index)))
    recordEvent('assessment_v2_chapter_start', { chapterId: this.chapterId, resumed: Boolean(Object.keys(this.session.answers).length) })
    if (index === 0) recordEvent('chapter_intro_view', { chapterId: this.chapterId })
  },
