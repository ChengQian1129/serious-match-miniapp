...
assert.deepEqual(store.participationTypesFromConsents(store.get().consents), ['interview'])
assert.equal(store.requiresContact(store.get().consents), true)
...
store.appendConsent('research_use', 'granted')
assert.deepEqual(store.participationTypesFromConsents(store.get().consents), ['research'])
assert.equal(store.requiresContact(store.get().consents), false)
store.appendConsent('offline_invitation', 'granted')
assert.deepEqual(store.participationTypesFromConsents(store.get().consents), ['research', 'offline'])
assert.equal(store.requiresContact(store.get().consents), true)
