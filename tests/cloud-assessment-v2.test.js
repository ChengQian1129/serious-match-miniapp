...
async function run() {
  ...
  result = await cloudFunction.main({ action: 'consentGrant', consentEvent: { eventId: 'consent-research-only', scope: 'research_use', createdAt: 1900 } })
  assert.equal(result.ok, true)
  result = await cloudFunction.main({ action: 'participantGet' })
  assert.equal(result.data.participant, null)
  result = await cloudFunction.main({ action: 'participantUpsert', participant, contact, idempotencyKey: 'research-only-write', clientUpdatedAt: 1950, schemaVersion: 'participant-2.1.0' })
  ...
  assert.equal(result.data.duplicateIgnored, true)
  const participantWrite = { idempotencyKey: 'participant-write-1', clientUpdatedAt: 2100, schemaVersion: 'participant-2.1.0' }
  result = await cloudFunction.main({ action: 'participantUpsert', participant, contact, ...participantWrite })
  assert.equal(result.ok, true)
  result = await cloudFunction.main({ action: 'participantUpsert', participant: Object.assign({}, participant, { displayName: '不应覆盖' }), contact, ...participantWrite })
  assert.equal(result.data.duplicateIgnored, true)
  assert.equal(result.data.participant.displayName, '测试参与者')
  result = await cloudFunction.main({ action: 'consentRevoke', consentEvent: { eventId: 'consent-research-before-case', scope: 'research_use', createdAt: 2050 } })
  assert.equal(result.ok, true)
  result = await cloudFunction.main({ action: 'participantUpsert', participant: Object.assign({}, participant, { displayName: '旧设备' }), contact, idempotencyKey: 'participant-write-old', clientUpdatedAt: 2000, schemaVersion: 'participant-2.1.0' })
  ...
  assert.equal(result.data.case.modelAvailable, false)
  assert.equal('reportSnapshot' in result.data.case, false)
  ...
  assert.equal('hypotheses' in result.data.preparation, false)
  assert.equal('phaseTwo' in result.data.preparation, false)
  assert.equal(rows('interview_cases').get(caseId).preparation.hypotheses.length > 0, true)
  ...
  result = await cloudFunction.main({ action: 'caseUpdateStatus', caseId, status: 'scheduled' })
  assert.equal(result.ok, true)
  assert.equal('reportSnapshot' in result.data.case, false)
  result = await cloudFunction.main({ action: 'caseUpdateStatus', caseId, status: 'in_progress' })
  assert.equal(result.ok, true)
  assert.equal('reportSnapshot' in result.data.case, false)
  ...
  assert.equal(result.code, 'BLIND_REVEAL_REQUIRED')
  result = await cloudFunction.main({ action: 'caseReveal', caseId })
  assert.equal(result.code, 'BLIND_REVEAL_REQUIRED')
  result = await cloudFunction.main({ action: 'independentObservationAppend', caseId, observation: { eventId: 'independent-1', topic: '冲突后的重新靠近', observation: '受访者先描述暂停后主动恢复对话的具体经历', alternativeExplanation: '可能只在低强度冲突中成立', interviewerConfidence: 'high', context: '盲法访谈', observedAt: 2990 } })
  assert.equal(result.ok, true)
  assert.equal(result.data.observation.eventType, 'independent_observation')
  assert.equal(result.data.observation.claimId, undefined)
  result = await cloudFunction.main({ action: 'caseReveal', caseId })
  assert.equal(result.ok, true)
  assert.equal(result.data.case.blindState, 'revealed')
  assert.ok(result.data.case.reportSnapshot.claims.length)
  assert.equal(result.data.case.preparation.phaseTwo.mode, 'model_assisted_review')
  result = await cloudFunction.main({ action: 'validationAppend', caseId, validationEvent: { eventId: 'validation-1', claimId: validationClaimId, verdict: 'confirmed', evidenceSummary: '具体事件中得到确认', interviewerConfidence: 'high', note: '具体事件中得到确认', context: '访谈', observedAt: 3000 } })
  assert.equal(result.ok, true)
  ...
  result = await cloudFunction.main({ action: 'caseUpdateStatus', caseId, status: 'reviewed' })
  ...
  result = await cloudFunction.main({ action: 'consentGrant', consentEvent: { eventId: 'consent-research-1', scope: 'research_use', createdAt: 4000 } })
  assert.equal(result.ok, true)
  currentOpenid = 'analyst-1'
  result = await cloudFunction.main({ action: 'deidentifiedExport' })
  assert.equal(result.ok, true)
  ...
  assert.equal(result.data.records[0].validations.find(item => item.eventType === 'claim_validation').result, 'confirmed')
  assert.equal(result.data.records[0].validations.some(item => item.eventType === 'independent_observation'), true)
  ...
  assert.equal(result.data.metrics.independentObservationCount, 1)
  ...
  currentOpenid = 'v2-user'
  result = await cloudFunction.main({ action: 'consentRevoke', consentEvent: { eventId: 'consent-research-2', scope: 'research_use', createdAt: 5000 } })
  assert.equal(result.data.consents.research_use.value, 'revoked')
  result = await cloudFunction.main({ action: 'consentRevoke', consentEvent: { eventId: 'consent-interview-2', scope: 'interview_contact', createdAt: 5010 } })
  assert.equal(result.ok, true)
  result = await cloudFunction.main({ action: 'participantGet' })
  ...
  assert.equal([...rows('interview_cases').values()].some(item => item.participantId === 'v2-user'), false)
  assert.equal([...rows('interview_validation_events').values()].some(item => item.participantId === 'v2-user'), false)
